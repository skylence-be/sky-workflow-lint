...
//	※※      ... ※※       loose doc/comment block between sections. Body is captured into Workflow.Docs.
//	※<id>※  ... ※※       doc block associated with the named step or node. Closer is always ※※.
//
...
func parseSky(r io.Reader) (*Workflow, error) {
	...
	for _, s := range sections {
		...
		wf.Docs = append(wf.Docs, DocBlock{
			StepID: s.name,
...
func scanSkySections(r io.Reader) ([]skySection, error) {
	...
	for scanner.Scan() {
		...
		// Doc block: capture body lines until the closing ※※.
		if inDoc {
			if trimmed == closeDoc {
				inDoc = false
				sections = append(sections, skySection{
		...
		if curSymbol == "" {
			...
			if id, ok := tryDocOpener(trimmed); ok {
				if containsForbidden(id) {
					return nil, skyerr.New(skyerr.ErrSkyFormat,
						fmt.Sprintf("line %d: doc block id %q contains forbidden character", lineNum, id))
				}
				inDoc = true
				docName = id
				docLine = lineNum
...
// tryDocOpener returns (id, true) when line is ※<id>※ with a non-empty id.
// ※※ (the loose opener/closer) is excluded by the len > 2*len(symDoc) guard.
func tryDocOpener(line string) (id string, ok bool) {
	if !strings.HasPrefix(line, symDoc) || !strings.HasSuffix(line, symDoc) {
		return "", false
	}
	if len(line) <= 2*len(symDoc) {
		return "", false
	}
	inner := line[len(symDoc) : len(line)-len(symDoc)]
